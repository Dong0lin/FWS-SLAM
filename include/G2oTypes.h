...
// 平面约束软边：一元边，惩罚地图点到给定平面（normal, offset）的距离
// 与 BA 无缝集成：不 fix 平面点，而是以 cost = ½·info·dist² 引导移动
class EdgePlaneConstraint : public g2o::BaseUnaryEdge<1, double, g2o::VertexSBAPointXYZ>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    ...
    void setPlane(const Eigen::Vector3d &normal, double offset, double information = 20.0)
    {
        mPlaneNormal = normal;
        mPlaneOffset = offset;
        setInformation(Eigen::Matrix<double,1,1>::Identity() * information);
    }
    ...
    void computeError()
    {
        ...
        _error[0] = mPlaneNormal.dot(v->estimate()) - mPlaneOffset;
    }

    virtual void linearizeOplus();

    Eigen::Vector3d mPlaneNormal;
    double mPlaneOffset;
};

// 关键帧级平面距离约束：惩罚关键帧相机中心到参考平面距离的漂移
// 一元边挂 VertexPose，数值 Jacobian，Info=8.0 覆盖几十帧的合力
class EdgeKFCamCenterToPlane : public g2o::BaseUnaryEdge<1, double, VertexPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeKFCamCenterToPlane() {}

    void setPlane(const Eigen::Vector3d &normal, double offset, double camHeightRef)
    {
        mPlaneNormal = normal;
        mPlaneOffset = offset;
        mCamHeightRef = camHeightRef;
        setInformation(Eigen::Matrix<double,1,1>::Identity() * 8.0);
    }
    ...
    void computeError()
    {
        ...
        _error[0] = mPlaneNormal.dot(v->estimate().tcw[0]) - mPlaneOffset - mCamHeightRef;
    }
    ...
    Eigen::Vector3d mPlaneNormal;
    double mPlaneOffset;
    double mCamHeightRef;
};

// 同上，挂 g2o::VertexSE3Expmap（用于 LocalBundleAdjustment 等标准 BA）
class EdgeKFCamCenterToPlaneSE3 : public g2o::BaseUnaryEdge<1, double, g2o::VertexSE3Expmap>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeKFCamCenterToPlaneSE3() {}
    ...
    void computeError()
    {
        ...
        Eigen::Vector3d Cw = v->estimate().inverse().translation();
        _error[0] = mPlaneNormal.dot(Cw) - mPlaneOffset - mCamHeightRef;
    }
