...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
                ...
                if(pMap && pMap->IsPlaneEstimated())
                {
                    ...
                    if(std::abs(ndir) > 1e-6f && ndir < 0)
                    {
                        ...
                        if(t_plane > 0 && t_frozen > 0)
                        {
                            ...
                            float alpha = 0;
                            float t_target = t_plane;

                            if(cosParallaxRays > 0.9998f)       alpha = 0.60f;
                            else if(cosParallaxRays > 0.9995f)  alpha = 0.45f;
                            else if(cosParallaxRays > 0.9990f)  alpha = 0.30f;
                            else if(cosParallaxRays > 0.9980f)  alpha = 0.15f;

                            float scaleRatio = t_triang / t_frozen;
                            float alpha_scale = 0;
                            // α = 1 - scaleRatio: 退化越严重，修正越强
                            // scaleRatio=0.3 → α=0.7 (推70%到冻结平面)
                            // scaleRatio=0.6 → α=0.4 (推40%)
                            // scaleRatio≥0.8 → α=0   (尺度健康，不干预)
                            if(scaleRatio < 0.8f)
                                alpha_scale = (1.0f - scaleRatio) * 0.85f;

                            if(alpha_scale > alpha) {
                                alpha = alpha_scale;
                                t_target = t_frozen;
                            }

                            // λ退化门: 全局尺度确认退化时(λ<0.7)移除视差门控
                            // 正常三角化(cos~0.5-0.9)也能触发修正, 对抗尺度漂移
                            float lambda = pMap->GetPlaneScaleLambda();
                            if(lambda > 0.01f && lambda < 0.7f && alpha_scale > alpha)
                                { alpha = alpha_scale; t_target = t_frozen; }
                            if(lambda > 0.01f && lambda < 0.4f && alpha_scale > 0)
                                alpha = std::max(alpha, alpha_scale * 1.2f);
            ...
            // 平面修正过的点: 预标记给BA更强的约束权重, 对抗被重投影拉回
            if(bPlaneCorrected) {
                pMP->mnPlaneID = 0;
                pMP->mfPlaneInfo = 3.0f;
            }
            if (bPointStereo)
                countStereo++;
