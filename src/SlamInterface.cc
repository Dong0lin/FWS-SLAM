                                              ...
                                              double timestamp)
{
    ...
    {
        // 缓存当前帧（DrawFrame 返回 BGR → 转为 RGB 给 Qt 显示）
        cv::Mat frameBGR = mImpl->pFrameDrawer->DrawFrame(mImpl->imageScale);
        cv::Mat frameRGB;
        ...
        mImpl->cachedWidth  = frameRGB.cols;
        mImpl->cachedHeight = frameRGB.rows;
        mImpl->cachedFrame.assign(frameRGB.data,
                                  frameRGB.data + frameRGB.total() * 3);
    }
...
int SlamInterface::GetPersistentBoxes(float* outBoxes, float* outPlaneNormal, float* outPlaneOffset, int maxCount)
{
    ...
    for (const auto& box : boxes) {
        ...
        if (count >= maxCount) break;
        outBoxes[count*8 + 0] = box.center.x();
        outBoxes[count*8 + 1] = box.center.y();
        outBoxes[count*8 + 2] = box.center.z();
        outBoxes[count*8 + 3] = box.width;
        outBoxes[count*8 + 4] = box.depth;
        outBoxes[count*8 + 5] = box.height;
        outBoxes[count*8 + 6] = (float)box.class_id;
        outBoxes[count*8 + 7] = (float)box.nObservations;
        count++;
    }
...
{
    if (mImpl->pSystem) {
        mImpl->pSystem->SaveTrajectoryEuRoC(dir + "/CameraTrajectory.txt");
        mImpl->pSystem->SaveKeyFrameTrajectoryEuRoC(dir + "/KeyFrameTrajectory.txt");
    }
}
...
void SlamInterface::SetVisualizationMode(bool showDynamic)
{
    if (!mImpl->pSystem) return;
    mImpl->pSystem->GetTracker()->SetShowDynamicVis(showDynamic);
}
