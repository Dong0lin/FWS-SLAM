...
void Viewer::Run()
{
    ...
    while(1)
    {
        ...
        // Get camera pose from MapDrawer (Eigen), convert to pangolin types
        Twc_eigen = mpMapDrawer->GetCurrentCameraPose();
        for (int i = 0; i<4; i++) {
            Twc.m[4*i]   = Twc_eigen(0,i);
            Twc.m[4*i+1] = Twc_eigen(1,i);
            Twc.m[4*i+2] = Twc_eigen(2,i);
            Twc.m[4*i+3] = Twc_eigen(3,i);
        }
        Ow.SetIdentity();
        Ow.m[12] = Twc_eigen(0,3);
        Ow.m[13] = Twc_eigen(1,3);
        Ow.m[14] = Twc_eigen(2,3);
        ...
        mpMapDrawer->DrawCurrentCamera(Twc_eigen);
        ...
        if(menuShowDetection3D) {
            mpMapDrawer->DrawDetection3Ds();
            if (!mpTracker->Is3DBoxDetectionEnabled())
                mpTracker->SetEnable3DBoxDetection(true);
        } else {
            if (mpTracker->Is3DBoxDetectionEnabled())
                mpTracker->SetEnable3DBoxDetection(false);
        }
