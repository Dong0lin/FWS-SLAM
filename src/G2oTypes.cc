...
void EdgeKFCamCenterToPlane::linearizeOplus()
{
    const VertexPose* v = static_cast<const VertexPose*>(_vertices[0]);
    ImuCamPose pose = v->estimate();
    double base = mPlaneNormal.dot(pose.tcw[0]);
    const double eps = 1e-6;

    for(int i = 0; i < 6; i++) {
        ImuCamPose posePert = pose;
        double pert[6] = {0, 0, 0, 0, 0, 0};
        pert[i] = eps;
        posePert.Update(pert);
        _jacobianOplusXi(0, i) = (mPlaneNormal.dot(posePert.tcw[0]) - base) / eps;
    }
}

void EdgeKFCamCenterToPlaneSE3::linearizeOplus()
{
    ...
    _jacobianOplusXi.block<1,3>(0,0).setZero();
    _jacobianOplusXi.block<1,3>(0,3) = -(R * mPlaneNormal).transpose();
}
