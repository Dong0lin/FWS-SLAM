...
# define MovingProbability 0.0  //默认移动概率
# define StaticProbability 1.0  //默认静态概率
# define MovingThreshold 0.5  //默认动态阈值
...
{
    mMovingProbability = MovingProbability;
    mStaticProbability = StaticProbability;
    mnObservedDynamic = 0;
    mFeatureStatus = STATIC;
    mnSemanticClass = -1;
    mnPlaneID = -1;
    mfPlaneDistance = 0.0f;
    mfPlaneInfo = 1.0f;
...
// 判断是否为动态地图点，移动概率大于阈值则为动态地图点，动态点返回true
bool MapPoint::IsDynamicMapPoint()
{
    unique_lock<mutex> lock(mMutexFeatures);
    if (mMovingProbability <= MovingThreshold) {
        return false;
    }
    return true;
}
...
void MapPoint::SetMovingProbability(const float& in_mp)
{
    ...
    mStaticProbability = 1.0f - in_mp;
    
    if (mMovingProbability > MovingThreshold) {
        mnObservedDynamic++;
        mFeatureStatus = DYNAMIC;
    }else if(mMovingProbability > 0)
    {
        mFeatureStatus = SEMANTIC;  
    }
}
...
MapPoint::FeatureStatus MapPoint::GetFeatureStatus()
{
    unique_lock<mutex> lock(mMutexFeatures);
