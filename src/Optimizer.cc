...
/**
 * @brief 光束法平差(Bundle Adjustment)核心函数
 * 
 * 该函数执行ORB-SLAM3中的光束法平差优化，同时优化关键帧位姿和地图点位置。
 * 支持单目、双目和多传感器（IMU/Body）观测，并可集成平面约束。
 * 
 * @param vpKFs 参与优化的关键帧集合
 * @param vpMP 参与优化的地图点集合
 * @param nIterations 优化迭代次数
 * @param pbStopFlag 外部停止标志指针（可选）
 * @param nLoopKF 回环关键帧ID，用于区分全局BA和局部BA
                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    // 使用Eigen线性求解器，处理6自由度位姿和3自由度点
    linearSolver = new g2o::LinearSolverEigen<g2o::BlockSolver_6_3::PoseMatrixType>();

    // 创建块求解器，6x3表示6自由度位姿顶点和3自由度点顶点
    g2o::BlockSolver_6_3 * solver_ptr = new g2o::BlockSolver_6_3(linearSolver);

    // 使用Levenberg-Marquardt优化算法
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    optimizer.setAlgorithm(solver);
    optimizer.setVerbose(false);  // 关闭详细输出
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        // 极弱 KF 级平面约束: 给 BA 一个微弱方向打破点-KF 尺度拔河
        if(!vSE3->fixed() && pMap->IsPlaneEstimated())
        {
            EdgeKFCamCenterToPlaneSE3* eKF = new EdgeKFCamCenterToPlaneSE3();
            eKF->setVertex(0, vSE3);
    ...
    // Huber鲁棒核的阈值：2D点对应自由度2，阈值为sqrt(5.99)；3D点对应自由度3，阈值为sqrt(7.815)
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        if(pMP->mnPlaneID >= 0 && pMap && pMap->IsPlaneEstimated()) {
            EdgePlaneConstraint* eP = new EdgePlaneConstraint();
...
/**
 * 位姿优化：使用当前帧与地图点的重投影误差优化相机位姿
 * 这是ORB-SLAM3中最重要的优化函数之一，用于精化跟踪过程中的相机位姿估计
 * 
 * @param pFrame 待优化的当前帧
 * @return int 优化后的内点数量
 */
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // 使用Levenberg-Marquardt优化算法（鲁棒性好）
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    optimizer.setAlgorithm(solver);

    int nInitialCorrespondences=0;  // 初始匹配点数量

    // 第二步：设置帧顶点（相机位姿）
    g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
    Sophus::SE3<float> Tcw = pFrame->GetPose();  // 获取当前帧的初始位姿
    vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(),Tcw.translation().cast<double>()));
    vSE3->setId(0);
    vSE3->setFixed(false);  // 位姿需要优化，不固定
    optimizer.addVertex(vSE3);

    // 第三步：设置地图点边（重投影误差边）
    const int N = pFrame->N;  // 当前帧的特征点数量

    // 存储各种类型的边和对应的索引
    vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose*> vpEdgesMono;  // 单目边
    vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPoseToBody *> vpEdgesMono_FHR;  // 双目右视图边
    vector<size_t> vnIndexEdgeMono, vnIndexEdgeRight;  // 边对应的特征点索引
    vpEdgesMono.reserve(N);
    vpEdgesMono_FHR.reserve(N);
    vnIndexEdgeMono.reserve(N);
    vnIndexEdgeRight.reserve(N);

    vector<g2o::EdgeStereoSE3ProjectXYZOnlyPose*> vpEdgesStereo;  // 立体边
    vector<size_t> vnIndexEdgeStereo;  // 立体边索引
    vpEdgesStereo.reserve(N);
    vnIndexEdgeStereo.reserve(N);

    // 鲁棒核函数的阈值（基于卡方分布）
    const float deltaMono = sqrt(5.991);   // 单目：95%置信度，2自由度
    const float deltaStereo = sqrt(7.815); // 立体：95%置信度，3自由度

    // 第四步：构建优化图（添加边）
    {
    unique_lock<mutex> lock(MapPoint::mGlobalMutex);  // 线程安全锁
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            float weight = pMP->mStaticProbability;
            
            // if(weight != 1.0f)
            //     cout << "当前点有权重：" << weight << endl;
            ...
            if(!pFrame->mpCamera2){
                ...
                {
                    nInitialCorrespondences++;  // 增加匹配计数
                    pFrame->mvbOutlier[i] = false;  // 重置异常点标记
                    ...
                    e->setMeasurement(obs);
                    const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];  // 金字塔层级的逆方差
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2*weight);  // 信息矩阵（动态概率加权）
            ...
            // 刚性体SLAM（双相机系统）
                ...
                if (i < pFrame->Nleft) {    // 左相机观测
                    ...
                    e->pCamera = pFrame->mpCamera2;  // 右相机模型
                    e->Xw = pMP->GetWorldPos().cast<double>();
    ...
    // 检查匹配点数量是否足够（至少3个点才能确定位姿）
    if(nInitialCorrespondences<3)
        return 0;

    // 第五步：执行4轮优化，每轮后重新分类内点/外点
    // 外点在下一轮优化中不参与，但最终可能被重新分类为内点
    const float chi2Mono[4]={5.991,5.991,5.991,5.991};   // 单目卡方阈值
    const float chi2Stereo[4]={7.815,7.815,7.815, 7.815}; // 立体卡方阈值
    const int its[4]={10,7,5,5};    // 每轮迭代次数

    int nBad=0;  // 异常点数量
    for(size_t it=0; it<4; it++)    // 4轮优化，每轮后更新异常点标记
    {
        // 更新顶点估计值
        Tcw = pFrame->GetPose();
        vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(),Tcw.translation().cast<double>()));
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            const float chi2 = e->chi2();  // 计算卡方值

            if(chi2>chi2Mono[it])
            {                
                pFrame->mvbOutlier[idx]=true;  // 标记为异常点
                e->setLevel(1);  // 降低边的权重
                nBad++;
            }
            else
            ...
            if(it==2)
                e->setRobustKernel(0);  // 最后两轮禁用鲁棒核函数
        }
    ...
    pFrame->SetPose(pose);  // 更新帧的位姿

    return nInitialCorrespondences-nBad;  // 返回内点数量
}

/**
 * 局部Bundle Adjustment优化：优化局部窗口内的关键帧位姿和地图点位置
 * 这是ORB-SLAM3中最重要的后端优化函数之一，用于精化局部地图的几何结构
 * 
 * @param pKF 当前关键帧（优化的起点）
 * @param pbStopFlag 停止标志指针（用于外部中断优化）
 * @param pMap 地图指针
 * @param num_fixedKF 输出：固定关键帧数量
 * @param num_OptKF 输出：优化关键帧数量
 * @param num_MPs 输出：地图点数量
 * @param num_edges 输出：边数量
 */
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    lLocalKeyFrames.push_back(pKF);  // 添加当前关键帧
    pKF->mnBALocalForKF = pKF->mnId;  // 标记为局部BA的关键帧
    Map* pCurrentMap = pKF->GetMap();  // 获取当前地图
    ...
    for(int i=0, iend=vNeighKFs.size(); i<iend; i++)
    {
        ...
        pKFi->mnBALocalForKF = pKF->mnId;  // 标记为局部BA的关键帧
        if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)  // 检查关键帧有效性
            lLocalKeyFrames.push_back(pKFi);  // 添加到局部关键帧列表
    }

    // 第二步：收集局部地图点（在局部关键帧中可见的地图点）
    num_fixedKF = 0;  // 初始化固定关键帧计数
    list<MapPoint*> lLocalMapPoints;
    set<MapPoint*> sNumObsMP;
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin() , lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
        if(pKFi->mnId==pMap->GetInitKFid())  // 检查是否为初始关键帧
        {
            num_fixedKF = 1;  // 初始关键帧需要固定
        }
        vector<MapPoint*> vpMPs = pKFi->GetMapPointMatches();  // 获取关键帧的地图点匹配
        for(vector<MapPoint*>::iterator vit=vpMPs.begin(), vend=vpMPs.end(); vit!=vend; vit++)
        {
            MapPoint* pMP = *vit;
            if(pMP)
                if(!pMP->isBad() && pMP->GetMap() == pCurrentMap)  // 检查地图点有效性
                {

                    if(pMP->mnBALocalForKF!=pKF->mnId)  // 避免重复添加
                    {
                        lLocalMapPoints.push_back(pMP);
                        pMP->mnBALocalForKF=pKF->mnId;  // 标记为局部BA的地图点
                    }
                }
        }
    }

    // 第三步：收集固定关键帧（能看到局部地图点但不在局部关键帧中的关键帧）
    list<KeyFrame*> lFixedCameras;
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        map<KeyFrame*,tuple<int,int>> observations = (*lit)->GetObservations();  // 获取地图点的观测关系
        for(map<KeyFrame*,tuple<int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            KeyFrame* pKFi = mit->first;

            // 检查关键帧是否已经标记为局部或固定关键帧
            if(pKFi->mnBALocalForKF!=pKF->mnId && pKFi->mnBAFixedForKF!=pKF->mnId )
            {                
                pKFi->mnBAFixedForKF=pKF->mnId;  // 标记为固定关键帧
                if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
                    lFixedCameras.push_back(pKFi);
            }
        }
    }
    num_fixedKF = lFixedCameras.size() + num_fixedKF;  // 更新固定关键帧数量


    // 检查固定关键帧数量，如果没有固定关键帧则中止优化
    if(num_fixedKF == 0)
    {
    ...
    linearSolver = new g2o::LinearSolverEigen<g2o::BlockSolver_6_3::PoseMatrixType>();  // 使用Eigen线性求解器

    g2o::BlockSolver_6_3 * solver_ptr = new g2o::BlockSolver_6_3(linearSolver);

    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    if (pMap->IsInertial())  // IMU地图需要更大的初始lambda值
    ...
    if(pbStopFlag)
        optimizer.setForceStopFlag(pbStopFlag);  // 设置外部停止标志

    unsigned long maxKFid = 0;  // 最大关键帧ID，用于地图点ID分配
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        vSE3->setFixed(pKFi->mnId==pMap->GetInitKFid());  // 初始关键帧固定
        optimizer.addVertex(vSE3);
        // 极弱 KF 级平面约束
        if(!vSE3->fixed() && pMap->IsPlaneEstimated())
        {
    ...
    num_OptKF = lLocalKeyFrames.size();  // 输出优化关键帧数量
    ...
    for(list<KeyFrame*>::iterator lit=lFixedCameras.begin(), lend=lFixedCameras.end(); lit!=lend; lit++)
    {
        ...
        vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(),Tcw.translation().cast<double>()));
        vSE3->setId(pKFi->mnId);
        vSE3->setFixed(true);  // 固定关键帧的位姿不优化
        optimizer.addVertex(vSE3);
        if(pKFi->mnId>maxKFid)
            maxKFid=pKFi->mnId;
        // 调试信息：记录固定关键帧
    ...
    // 第七步：设置地图点顶点和边
    const int nExpectedSize = (lLocalKeyFrames.size()+lFixedCameras.size())*lLocalMapPoints.size();  // 预估边数量

    // 预分配各种边的存储空间
    vector<ORB_SLAM3::EdgeSE3ProjectXYZ*> vpEdgesMono;  // 单目边
    vpEdgesMono.reserve(nExpectedSize);

    vector<ORB_SLAM3::EdgeSE3ProjectXYZToBody*> vpEdgesBody;  // 右相机边
    vpEdgesBody.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFMono;  // 单目边对应的关键帧
    vpEdgeKFMono.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFBody;  // 右相机边对应的关键帧
    vpEdgeKFBody.reserve(nExpectedSize);

    vector<MapPoint*> vpMapPointEdgeMono;  // 单目边对应的地图点
    vpMapPointEdgeMono.reserve(nExpectedSize);

    vector<MapPoint*> vpMapPointEdgeBody;  // 右相机边对应的地图点
    vpMapPointEdgeBody.reserve(nExpectedSize);

    vector<g2o::EdgeStereoSE3ProjectXYZ*> vpEdgesStereo;  // 立体边
    vpEdgesStereo.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFStereo;  // 立体边对应的关键帧
    vpEdgeKFStereo.reserve(nExpectedSize);

    vector<MapPoint*> vpMapPointEdgeStereo;  // 立体边对应的地图点
    vpMapPointEdgeStereo.reserve(nExpectedSize);

    // 鲁棒核函数阈值（基于卡方分布）
    const float thHuberMono = sqrt(5.991);   // 单目：95%置信度，2自由度
    const float thHuberStereo = sqrt(7.815); // 立体：95%置信度，3自由度
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        g2o::VertexSBAPointXYZ* vPoint = new g2o::VertexSBAPointXYZ();
        vPoint->setEstimate(pMP->GetWorldPos().cast<double>());
        int id = pMP->mnId+maxKFid+1;  // 地图点ID从maxKFid+1开始
        vPoint->setId(id);
        vPoint->setMarginalized(true);  // 地图点需要边缘化
        optimizer.addVertex(vPoint);
        if(pMP->mnPlaneID >= 0 && pMap && pMap->IsPlaneEstimated()) {
            EdgePlaneConstraint* eP = new EdgePlaneConstraint();
            eP->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
        ...
        const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();  // 获取地图点的所有观测
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)  // 检查关键帧有效性
            {
                const int leftIndex = get<0>(mit->second);  // 左视图特征点索引
                ...
                if(leftIndex != -1 && pKFi->mvuRight[get<0>(mit->second)]<0)
                {
                    ...
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));  // 地图点顶点
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));  // 关键帧顶点
                    e->setMeasurement(obs);  // 观测数据
                    const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave];  // 金字塔层级的逆方差
                    ...
                    e->pCamera = pKFi->mpCamera;  // 设置相机模型
                ...
                else if(leftIndex != -1 && pKFi->mvuRight[get<0>(mit->second)]>=0)// 立体观测
                {
                    const cv::KeyPoint &kpUn = pKFi->mvKeysUn[leftIndex];
                    Eigen::Matrix<double,3,1> obs;
                    const float kp_ur = pKFi->mvuRight[get<0>(mit->second)];  // 右视图横坐标
                    obs << kpUn.pt.x, kpUn.pt.y, kp_ur;
                ...
                // 双相机系统：处理右相机观测
                if(pKFi->mpCamera2){
                    int rightIndex = get<1>(mit->second);  // 右视图特征点索引

                    if(rightIndex != -1 ){
                        rightIndex -= pKFi->NLeft;  // 调整右视图索引
                        ...
                        e->pCamera = pKFi->mpCamera2;  // 设置右相机模型
    ...
    num_edges = nEdges;  // 输出边数量
    num_MPs = nPoints;   // 输出地图点数量
    ...
    // 第八步：执行优化
    optimizer.initializeOptimization();
    optimizer.optimize(10);  // 执行10次迭代优化
    ...
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        ...
        // 卡方检验和深度有效性检查
        if(e->chi2()>5.991 || !e->isDepthPositive())  // 单目：卡方阈值5.991
        {
            KeyFrame* pKFi = vpEdgeKFMono[i];
            vToErase.push_back(make_pair(pKFi,pMP));  // 标记需要删除的匹配
        }
    }
    ...
    for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
    {
        ...
        if(e->chi2()>7.815 || !e->isDepthPositive())  // 立体：卡方阈值7.815
        {
    ...
    if(!vToErase.empty())
    {
        for(size_t i=0;i<vToErase.size();i++)
        {
            ...
            pKFi->EraseMapPointMatch(pMPi);  // 从关键帧中删除地图点匹配
            pMPi->EraseObservation(pKFi);    // 从地图点中删除观测关系
        }
    }
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        MapPoint* pMP = *lit;
        g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnId+maxKFid+1));
        pMP->SetWorldPos(vPoint->estimate().cast<float>());  // 设置优化后的位置
        pMP->UpdateNormalAndDepth();  // 更新法向量和深度信息
    }

    // 增加地图变化索引（通知其他线程地图已更新）
    pMap->IncreaseChangeIndex();
}
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF, bool *pbStopFlag)
{
    ...
    for(unsigned int i=0; i < vpMPs.size(); ++i)
    {
        ...
        if(pMPi->mnPlaneID >= 0 && pCurrentMap && pCurrentMap->IsPlaneEstimated()) {
            EdgePlaneConstraint* eP = new EdgePlaneConstraint();
            eP->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
